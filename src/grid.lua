...
Grid.width = 24
Grid.height = 16
Grid.cellSize = 10 -- Make grid cells very small for high resolution
Grid.cells = {}
...
local cellColors = {
    ...
    trail  = {1, 0.8, 0.2},
    enemy  = {0.8, 0.2, 0.2},
    island = {0.1, 0.9, 0.5}, -- צבע ייחודי לאי
}
...
---
-- Attempts to close an area based on a trail (cell-based).
-- @param trail List of cell coordinates
function Grid:closeArea(trail)
    ...
    -- Only claim regions with no enemy inside (classic Qix/Xonix logic)
    for lbl, cells in pairs(labelToCells) do
        if not labelHasEnemy[lbl] then
            for _, cell in ipairs(cells) do
                if self.cells[cell.i] and self.cells[cell.i][cell.j] and self.cells[cell.i][cell.j] ~= 'claimed' then
            ...
            -- Optional: show warning or feedback for failed closure
            if self.player then self.player.failedClosureWarning = 1.0 end
        end
    end
            ...
            for j=2, self.height-1 do
                if self.cells[i][j] == 'claimed' then
                    for _,d in ipairs({{1,1},{-1,1},{1,-1},{-1,-1}}) do
                        ...
                        if self:isInside(ni, nj) and self.cells[ni][nj] ~= 'claimed' then
                            if self.cells[i][nj] == 'claimed' and self.cells[ni][j] == 'claimed' then
...
function Grid:setSizeToWindow(ww, wh, margin)
    ...
    local targetGridW = 32 -- more cells horizontally
    local targetGridH = 22 -- more cells vertically
    local csW = math.floor(availableWidth / targetGridW)
    local csH = math.floor(availableHeight / targetGridH)
    self.cellSize = math.max(3, math.min(csW, csH), 4) -- even smaller
